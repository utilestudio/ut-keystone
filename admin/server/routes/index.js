...
module.exports = function IndexRoute (req, res) {
    ...
    lists[key].singleton = list.singleton;
	});
  ...
  var singletonPromises = [];
	_.forEach(keystone.lists, function (list, key) {
    if(list.singleton) {
      var singletonPromise = new Promise(function(resolve, reject) {
        list.model
          .findOne()
          .exec()
          .then(function(doc) {
      ...
      singletonPromises.push(
        singletonPromise 
      );
    }
	});

  Promise.all(singletonPromises).then(function(results) {
    var singletons = [];
